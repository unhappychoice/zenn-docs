...
:::message
🖼 **TODO**: shell hook → splashboard ( cache 読み + 描画 ) → fetch-only spawn のシーケンス図
:::
...
shell hook ──▶ splashboard
                 │
                 ▼
              $HOME/.splashboard/cache/<key>.json を読む  (1 widget 1 ファイル)
              realtime な widget はプロセス内で計算する
              1 回だけ描画する
                 │
                 ▼
              `splashboard fetch-only` を detached で spawn する
              ターミナルをシェルに返す
```
...
:::message
📸 **TODO**: `splashboard cache list` の実行結果スクリーンショット ( widget ごとに age / TTL / kind が並んだ表 )
:::
