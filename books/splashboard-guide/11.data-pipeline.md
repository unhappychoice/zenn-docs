...
:::message
🖼 **TODO**: 4 段パイプライン ( Config → Fetcher → Shape → Renderer ) を絵にした図 ( 下の ASCII 図をきれいに作画したもの )
:::
...
```
   dashboard.toml
        │
   ┌────▼────┐
   │ Config  │  TOML を解釈して widget の集合 + row レイアウトへ
   └────┬────┘
        │
   ┌────▼────┐
   │ Fetcher │  widget ごとに registry から fetcher を引き、 Payload を返す
   └────┬────┘
        │
   ┌────▼────┐
   │  Shape  │  Payload の型。 renderer との互換性をここで判定
   └────┬────┘
        │
   ┌────▼────┐
   │Renderer │  layout が決めた Rect の上に Payload を描き込む
   └─────────┘
```
...
:::message
🖼 **TODO**: row / child から Rect が計算されるレイアウト図 ( 下の ASCII を絵にしたもの )
:::
