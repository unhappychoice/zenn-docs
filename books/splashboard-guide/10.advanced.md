...
![basic_read_store × gauge_line: `store/habit.json` を読んで `habit · 6 / 10 日` を描画した状態](/images/splashboard-guide/demo-habit-gauge.png)

過去 10 日のうち何日継続できたかを 1 つのゲージで出す例。
...
{ "value": 0.6, "label": "habit · 6 / 10" }
```

`$HOME/.splashboard/store/habit.json` を毎晩 cron で書き換える簡単なスクリプトを置いておくだけで、シェルを開くたびに今の達成率がゲージで見えます。
...
![basic_numbers × chart_sparkline ( bars / line ) の例](/images/splashboard-guide/demo-sparkline.png)
...
:::message
📸 **TODO**: ワイドモニタ ( 200 × 60 程度 ) で広々と組んだ自作 home dashboard の screenshot
:::
...
逆に小さい画面で削れるように組みたい場合は、 `[[row]]` ごとに `min` / `max` をうまく使うか、 大きい preset と小さい preset を 2 つ持って `splashboard install` で切り替える方が簡単です。

## 季節 / 期間限定の splash

![clock_countdown × text_ascii (blocks): `to next year: 227d 8h` を巨大表示](/images/splashboard-guide/demo-countdown.png)

`clock_countdown` を使うと、特定の日付までの残り時間を出せます。年末カウントダウン dashboard の例。
