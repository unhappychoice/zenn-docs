...
`render` を省略すると、その fetcher の出力 shape にとって自然なデフォルト renderer が選ばれます。最初はとりあえず省略して動かしてみて、見た目を変えたくなったら明示する流れが楽です。
...
![project_github preset: figlet ヒーロー + サブタイトル + 複数セクションのレイアウト ( 下の TOML 例と近い構成 )](/images/splashboard-guide/project_github.png)
...
これで上からロゴ、サブタイトル、空白、 2 列 panel ( commits / released ) という構成になります。 4 つの widget と 4 つの row だけ。 splash としては十分なレイアウトです。
